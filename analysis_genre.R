...
env$data<-read.csv("msd_mean_pop.csv")
env$data<-read.table(file="msd_genre_dataset.txt",header=TRUE,sep=",",comment="#",quote=NULL,fill=TRUE)
...
env$datafit<-rpart(genre~.,data=env$data[env$train,c(env$input,env$target)],method="class")
print("Finished classifying using Decision Tree")

#result for decision tree

env$datapr<-predict(env$datafit,newdata=env$data[env$test,c(env$input,env$target)],type="class")

print(length(env$data))
print(length(env$test))
print(length(env$data[env$test,c(env$input, env$target)]))
print(length(env$datapr))

perdata<-pcme(env$data[env$test,c(env$input,env$target)]$genre,env$datapr)
round(perdata, 2)
cat(100*round(1-sum(diag(perdata),na.rm=TRUE),2))

errormatdata<-table(env$data[env$test,c(env$input,env$target)]$genre,env$datapr,useNA="ifany",dnn=c("Actual","Predicted"))
write.table(errormatdata,file="genre_error_matrix_data.txt")

#randon forest
print("Classifying using Random Forest...")
env$rffit<-randomForest::randomForest(genre~.,data=env$data[env$train,c(env$input,env$target)],ntree=5,mtry=2,importance=TRUE,na.action=randomForest::na.roughfix,replace=FALSE)
print("Finished classifying using Random Forest")

#results for random Forest
env$rfpr<-predict(env$rffit,newdata=env$data[env$test,c(env$input,env$target)],type="class")


#Plot Error Matrix

errormatrf<-table(env$data[env$test,c(env$input,env$target)]$genre,env$rfpr,useNA="ifany",dnn=c("Actual", "Predicted"))
write.table(errormatrf,file="genre_error_matrix_rf.txt")
perrf<-pcme(env$data[env$test,c(env$input,env$target)]$genre,env$rfpr)
round(perdata,2)
...
env$svmfit<-svm(genre~.,data=env$data[env$train,c(env$input,env$target)])
print("Finished classifying using SVM")

#Evaluate Results for SVM
env$svmpr<-predict(env$svmfit,newdata=env$data[env$test, c(env$input, env$target)], type="class")
errormatsvm<-table(env$data[env$test,c(env$input,env$target)]$genre,env$svmpr,useNA="ifany",dnn=c("Actual","Predicted"))
write.table(errormatsvm,file="genre_error_matrix_svm.txt")
persvm<-pcme(env$data[env$test,c(env$input,env$target)]$genre,env$svmpr)
round(persvm,2)
