...
env$dtfit<-rpart(popular~.,data=env$dt[env$train,c(env$input,env$target)],method="class")
print("Finished classifying using Decision Tree")

#svm

print("Classifying using SVM...")
env$svmfit<-svm(popular~.,data=env$dt[env$train,c(env$input,env$target)])
print("Finished classifying using SVM")

#random forest

print("Classifying using Random Forest...")
env$rffit<-randomForest::randomForest(popular~.,data=env$dt[env$train,c(env$input,env$target)],ntree=500,mtry=2,importance=TRUE,na.action=randomForest::na.roughfix,replace=FALSE)
print("Finished classifying using Random Forest")

#plot and graphs

print("Obtain the response from the classifyers...")
env$dtpr<-predict(env$dtfit,newdata=na.omit(env$dt[env$test,c(env$input,env$target)]))
env$svmpr<-predict(env$svmfit,newdata=na.omit(env$dt[env$test,c(env$input,env$target)]))
env$rfpr<-predict(env$rffit,newdata=na.omit(env$dt[env$test,c(env$input,env$target)]))
